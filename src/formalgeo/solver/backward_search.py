...
from formalgeo.tools import output, debug_print
...
def get_p2t_map_bw(t_info, parsed_theorem_GDL):
    ...
    for t_name in parsed_theorem_GDL:
        if t_name in t_info and (
                t_info[t_name][1] == 0 or t_info[t_name][0] == 3
        ):  # skip no used and diff t
            continue
...
class GoalFinder:
    ...
    def find_all_sub_goals(self, predicate, item, problem):
        ...
        if predicate == "Equation":  # algebra goal
            attr_to_paras = {}
            for sym in EqKiller.get_minimum_syms(
                    [item], list(problem.condition.simplified_equation)
            ):
            ...
            for attr in attr_to_paras:
                for t_name, t_branch in self.p2t_map[attr]:
                    ...
                    t_paras = []
                    for t_attr, attr_vars in self.parsed_theorem_GDL[t_name]["body"][
                        t_branch
                    ]["attr_in_conclusions"]:
                        if attr != t_attr:
                            continue
                        ...
                        for para in attr_to_paras[attr]:
                            t_para = [
                                v if v not in attr_vars else para[attr_vars.index(v)]
                                for v in t_vars
                            ]
            ...
            for t_name, t_branch in self.p2t_map[predicate]:
                ...
                for t_predicate, item_vars in self.parsed_theorem_GDL[t_name]["body"][
                    t_branch
                ]["conclusions"]:
                    ...
                    t_vars = copy.copy(self.parsed_theorem_GDL[t_name]["vars"])
                    t_para = [
                        v if v not in item_vars else item[item_vars.index(v)]
                        for v in t_vars
                    ]
        ...
        for t_para in t_paras:
            ...
            for per_para in permutations(points, len(vacant_index)):
                result = [
                    (
                        t_para[i]
                        if i not in vacant_index
                        else per_para[vacant_index.index(i)]
                    )
                    for i in range(len(t_para))
                ]
    ...
    def gen_sub_goals(self, theorem_and_para, problem):
        ...
        for t in theorem_and_para:
            ...
            for t_para in theorem_and_para[t]:
                ...
                sub_goals = []
                passed = True
                for predicate, item_vars in self.parsed_theorem_GDL[t_name]["body"][
                    t_branch
                ]["products"]:
                    item = tuple(letters[i] for i in item_vars)
                    if not (
                            problem.ee_check(predicate, item)
                            and problem.fv_check(predicate, item)
                    ):
                        passed = False
                        break
                ...
                for _, tree in self.parsed_theorem_GDL[t_name]["body"][t_branch][
                    "algebra_constraints"
                ]:
...
class Node:
    def __init__(self, super_node, problem, predicate, item, node_map, finder, debug):
        ...
        if predicate == "Equation":  # process 1
            ...
            if predicate in [
                ...
                "Collinear",
                "Cocircular",
            ] and item not in self.problem.condition.get_items_by_predicate(predicate):
    ...
    def check_goal(self):  # process 1
        ...
        if self.predicate == "Equation":
            ...
            if self.item not in self.problem.condition.get_items_by_predicate(
                    self.predicate
            ):
                return False
            self.state = NodeState.success
            self.premise = [
                self.problem.condition.get_id_by_predicate_and_item(
                    self.predicate, self.item
                )
            ]
    ...
    def expand(self, search_stack):  # process 1
        ...
        results = self.finder.find_all_sub_goals(
            self.predicate, self.item, self.problem
        )
...
class SuperNode:
    ...
    def expand(self):
        ...
        for i in range(len(self.nodes)):
            ...
            debug_print(
                self.debug,
                "(pid={},depth={},branch={}/{},nodes={}/{}) Expanding Node ({}, {})".format(
                    self.problem.parsed_problem_CDL["id"],
                    self.pos[0],
                    self.pos[1],
                    SuperNode.snc[self.pos[0]],
                    i + 1,
                    len(self.nodes),
                    self.nodes[i].predicate,
                    self.nodes[i].item,
                ),
            )
    ...
    def apply_theorem(self):
        ...
        for predicate, item in gpl["products"] + gpl["logic_constraints"]:
            ...
            if has_item:
                premises.append(
                    self.problem.condition.get_id_by_predicate_and_item(predicate, item)
                )
...
class BackwardSearcher:
    ...
    def __init__(
        ...
        self.parsed_theorem_GDL = parse_theorem_gdl(
            theorem_GDL, self.parsed_predicate_GDL
        )
        ...
        self.node_map = None
        self.finder = GoalFinder(
            self.parsed_theorem_GDL, get_p2t_map_bw(t_info, self.parsed_theorem_GDL)
        )
    ...
    def init_search(self, problem_CDL):
        ...
        self.problem.load_problem_by_fl(
            self.parsed_predicate_GDL,
            self.parsed_theorem_GDL,
            parse_problem_cdl(problem_CDL),
        )  # load problem
        EqKiller.solve_equations(self.problem)
        self.problem.step(
            "init_problem", time.time() - s_start_time
        )  # save applied theorem and update step
    ...
    def search(self):
        ...
        if self.strategy == "bfs":
            while self.root.state not in [NodeState.success, NodeState.fail]:
                ...
                debug_print(
                    self.debug,
                    "(pid={},depth={},branch={}/{}) Expanding SuperNode Start".format(
                        pid,
                        super_node.pos[0],
                        super_node.pos[1],
                        SuperNode.snc[super_node.pos[0]],
                    ),
                )
                super_node.expand()
                ...
                debug_print(
                    self.debug,
                    "(pid={},depth={},branch={}/{}) Checking Node Start".format(
                        pid,
                        super_node.pos[0],
                        super_node.pos[1],
                        SuperNode.snc[super_node.pos[0]],
                ...
                debug_print(
                    self.debug,
                    "(pid={},depth={},branch={}/{}) Checking Node End (timing={:.4f})".format(
                        pid,
                        super_node.pos[0],
                        super_node.pos[1],
                        SuperNode.snc[super_node.pos[0]],
            ...
            while self.root.state not in [NodeState.success, NodeState.fail]:
                ...
                if len(self.search_stack) == 0:
                    break
                super_node = self.search_stack.pop(
                    random.randint(0, len(self.search_stack) - 1)
                )
                self.step_size += 1
                start_step_count = self.problem.condition.step_count
            ...
            while self.root.state not in [NodeState.success, NodeState.fail]:
                ...
                if (
                        len(self.search_stack) > self.beam_size
                ):  # select branch with beam size
                    search_stack = []
                    for i in random.sample(
                            range(len(self.search_stack)), self.beam_size
                    ):
        ...
        if self.problem.goal.solved:
            debug_print(
                self.debug,
                "(pid={}) End Searching".format(self.problem.parsed_problem_CDL["id"]),
            )
            _, _, seqs = output(self.problem)
            return True, seqs
    ...
    def check_node(self, start_step_count):
            ...
            for _id in self.problem.condition.ids_of_step[step]:
                if self.problem.condition.items[_id][0] == "Equation":
                    ...
                    for simp_eq in self.problem.condition.simplified_equation:
                        ...
                        if (
                                _id
                                not in self.problem.condition.simplified_equation[simp_eq]
                        ):
                            continue
                        related_eqs.append(simp_eq)
                else:
                    predicate, item = self.problem.condition.items[_id][0:2]
                    if (predicate, item) not in self.node_map or (
                            predicate,
                            item,
                    ) in related_pres:
                        continue
        ...
        for sym in EqKiller.get_minimum_syms(
                related_eqs, list(self.problem.condition.simplified_equation)
        ):
